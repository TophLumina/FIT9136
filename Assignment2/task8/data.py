...
def load_database(reminders_file, active_file, dismissed_file) -> None:
        ...
        for line in rf:
            ...
            if remainder_detail[0].isdigit():
                reminders_database.append(
                    (int(remainder_detail[0]), remainder_detail[1][1:-1])
                )

    reminders_active_database.clear()
    with open(active_file, "r") as af:
        for line in af:
            active_detail = line.strip().split(",")
            if active_detail[0].isdigit():
                reminders_active_database.append(
                    (
                        int(active_detail[0]),
                        int(active_detail[1]),
                        dt.datetime.strptime(active_detail[2], "%Y-%m-%d %H:%M:%S"),
                    )
                )
    ...
    with open(dismissed_file, "r") as df:
        for line in df:
            dismiss_detail = line.strip().split(",")
            if dismiss_detail[0].isdigit():
                reminders_dismissed_database.append(
                    (
                        int(dismiss_detail[0]),
                        int(dismiss_detail[1]),
                        dt.datetime.strptime(dismiss_detail[2], "%Y-%m-%d %H:%M:%S"),
                    )
                )
        pass
...
def get_active_reminders() -> list:
    return [item for item in get_all_reminder() if item.is_active()]


def get_past_reminders() -> list:
    return [item for item in get_all_reminder() if item.is_passed()]


def get_future_reminders() -> list:
    return [item for item in get_all_reminder() if item.is_future()]
